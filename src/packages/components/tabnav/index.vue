<template>
  <div class="em-tabnav">
    ...
    <div class="em-tabnav-control">
      <el-dropdown @command="cmd => handleCommand(cmd)">
        <span class="em-tabnav-control-btn">
          <em-icon name="angle-double-down" />
        </span>
        <el-dropdown-menu slot="dropdown">
          <el-dropdown-item command="Left">
            <em-icon name="arrow-left" class="em-tabnav-control-icon" />关闭左侧
          </el-dropdown-item>
          <el-dropdown-item command="Right">
            <em-icon name="arrow-right" class="em-tabnav-control-icon" />关闭右侧
          </el-dropdown-item>
          <el-dropdown-item command="Other">
            <em-icon name="other" class="em-tabnav-control-icon" />关闭其他
          </el-dropdown-item>
          <el-dropdown-item command="All">
            <em-icon name="app" class="em-tabnav-control-icon" />全部关闭
          </el-dropdown-item>
        </el-dropdown-menu>
      </el-dropdown>
    </div>
    <div
      ref="contextmenu"
      class="em-tabnav-contextmenu"
      v-show="contextmenu.visible"
      :style="{ top: contextmenu.top, left: contextmenu.left }"
    >
      <ul>
        <li @click.stop="onRefresh">
          <em-icon name="refresh" />重新载入
        </li>
        <li @click.stop="onClose('')">
          <em-icon name="close" />关闭
        </li>
        <li @click.stop="onClose('Left')">
          <em-icon name="arrow-left" />关闭左侧
        </li>
        <li @click.stop="onClose('Right')">
          <em-icon name="arrow-right" />关闭右侧
        </li>
        <li @click.stop="onClose('Other')">
          <em-icon name="other" />关闭其它
        </li>
        <li @click.stop="onClose('All')">
          <em-icon name="app" />关闭全部
        </li>
      </ul>
    </div>
  </div>
