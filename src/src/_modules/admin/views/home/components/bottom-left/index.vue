<template>
  <em-panel v-bind="panel">
    ...
    <ve-line :data="chartData" :extend="chartExtend" height="100%" :settings="chartSettings"></ve-line>
  </em-panel>
</template>
<script>
export default {
  ...
  methods: {
    getData() {
      ...
      for (let index = 1; index < 31; index++) {
        this.chartData.rows.push({ 日期: index, 提报数量: Math.floor(Math.random() * (100 - 200)) + 100 })
      }
    },
    refresh() {
      this.getData()
