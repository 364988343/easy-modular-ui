<template>
   <em-panel page>
    <em-list-select ref="listSel" v-bind="listSel" @change="onChange">
      <template v-slot:btn>
        <el-input v-model="result" class="input-with-select" style="width: 400px">
          <el-button slot="append" icon="el-icon-search" @click="$refs.listSel.show()"></el-button>
        </el-input>
      </template>
...
export default {
  ...
  methods: {
    ...
    onChange(rows) {
      this.result = rows.map((m) => m.name).join(',')
    }
  }
}
</script>
